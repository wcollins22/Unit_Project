...
def home(request,u_name):
    ...
    context = {
        ...
        "students" : {
            ...
            "RJay Pickering" : Student("RJay Pickering", "I like to play video games and fix broken vehicles.  i use to go to the gym a lot and i love working around a farm", "videogame designer", "student/farmer", "Pontotoc, MS"),
...
def homepage_view(request):
    ...
    if form.is_valid():
        name = form.cleaned_data["name"]
        for name2, info in students.items():
            if name2 == name:
                the_name = info.u_name
                bio = info.bio
                fav_genre = info.fav_genre
                dream = info.dream
                job = info.job
                hometown = info.hometown
        return render(request, "home.html", {"the_name":the_name, "bio":bio, "fav_genre":fav_genre, "dream":dream, "job":job, "hometown":hometown})
